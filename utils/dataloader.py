...
class SVFNDDataset(Dataset):
    def __init__(self, path_vid, token, dataset, datamode='title+ocr'):
        ...
        if self.dataset == 'fakesv':
            ...
            self.maefeapath = 'data/fea/fakesv/video_clip'
            self.hubert_path = 'data/fea/fakesv/hubert_ems/'
            ...
            self.hubert_path = 'data/fea/fakett/hubert_ems/'
...
def pad_frame_sequence_fakesv_audio(seq_len, lst, modality):
    attention_masks = []
    result = []
    for item in lst:
        if modality == 'video':
...
def fakesv_collate_fn(batch):
    ...
    audio_feas, audiofeas_masks = pad_frame_sequence_fakesv_audio(num_audioframes, audio_feas, modality='audio')
